...
\begin{definition}[Entity Matching]
  Given two sets of entities  $E_1$  and $E_2$, the task is to determine, for every pair $(e_1, e_2) \in E_1 \times E_2$, whether $e_1$ and $e_2$ refer to the same real-world entity. It is a binary classification task that determines whether a candidate pair is a match $(+1)$ or non-match $(-1)$.
\end{definition}

Consider Entities as products in this scenario. Let's say we have two sets of product listings, $P_1$ and $P_2$, sourced from two different e-commerce platforms --- say Shopee and Lazada respectively. Each listing carries attributes such as \texttt{p-name}, \texttt{p-description}, \texttt{p-year}, and \texttt{p-price}. The goal of Entity Matching in this scenario is to decide, for every pair $(p_1, p_2) \in P_1 \times P_2$, whether $p_1$ and $p_2$ refer to the same real-world product.

What makes this non-trivial is that even a genuinely matching pair may disagree on attribute values. For instance, one platform may list a product as \textit{``Sony WH-1000XM5''} while another lists it as \textit{``Sony Noise Cancelling Headphones XM5 Series''}. They are referring to the same product but with different representations. It is therefore insufficient to simply compare entities on this level. To attain maximum accuracy in entity matching, it falls upon a human expert to manually inspect each pair and make a verdict. However, with thousands of listings on each platform, the number of pairs 
...
To address the Entity Matching Problem, a common approach is to make it a challenge of classification. This is done by passing datasets through what is known broadly as the ER Pipeline consisting of three key steps: \textbf{Blocking}, \textbf{Comparison}, and \textbf{Matching}.  The first two steps serve as preprocessing stages that progressively reduce and structure the problem, while the third step is what Entity Matching entails and is the primary focus of the paper. Before diving deep into the third step, we will go over the first two steps in brief.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{../pipeline.png}
  \caption{The Entity Resolution Pipeline. The \textbf{Matching} stage is what will be covered in detail.}
...
\end{sloppypar}
  ...
  \textbf{Locality Sensitive Hashing (LSH)}. It groups similar pairs into candidate buckets based on hash collisions, this was covered in Week 3 of this course.
\end{remark}
